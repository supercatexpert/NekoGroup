...
static gboolean ng_cmd_member_list(const gchar *jid, const gchar *key)
{
    ...
    if(key!=NULL)
        g_string_append_printf(result_string, _("The members who's "
            "nicks contain \'%s\':\n"), key);
    else
        g_string_append_printf(result_string, _("All members:\n"));
    if(table!=NULL)
    {
    ...
    g_string_append_printf(result_string,
        ngettext(_("%u member listed"), _("%u members listed"), count), count);
    ng_core_send_message(jid, result_string->str);
    g_string_free(result_string, TRUE);
    return TRUE;
}
...
static gboolean ng_cmd_member_online(const gchar *jid, const gchar *key)
{
    ...
    real_time = ng_uitls_get_real_time();
    if(key!=NULL)
        g_string_append_printf(result_string, _("The online member who's "
            "nicks contain \'%s\':\n"), key);
    else
        g_string_append_printf(result_string, _("All online members:\n"));
    ...
    g_string_append_printf(result_string,
        ngettext(_("%u member listed"), _("%u members listed"), count),
        count);
    ng_core_send_message(jid, result_string->str);
    g_string_free(result_string, TRUE);
    return TRUE;
    ...
    const gchar *length)
{
    ...
    {
        ng_core_send_message(jid, _("Root members cannot be banned!"));
        g_free(target_jid);
        return TRUE;
    }
    if(level==NG_DB_USER_PRIVILEGE_POWER &&
    ...
    {
        ng_core_send_message(jid, _("You cannot ban power members or "
            "root members!"));
        g_free(target_jid);
        return TRUE;
    }
...
static gboolean ng_cmd_member_kick(const gchar *jid, const gchar *victim)
{
    ...
    if(victim_level==NG_DB_USER_PRIVILEGE_ROOT)
    {
        ng_core_send_message(jid, _("Root members cannot be kicked!"));
        g_free(target_jid);
        return TRUE;
    }
    if(level==NG_DB_USER_PRIVILEGE_POWER &&
        victim_level>=NG_DB_USER_PRIVILEGE_POWER)
    {
        ng_core_send_message(jid, _("You cannot kick power members or "
            "root members!"));
        g_free(target_jid);
        return TRUE;
    }
